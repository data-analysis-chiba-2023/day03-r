...
2. Under the `File` menu, click on `New Project`. Choose `New Directory`, then
  `New Project`.
3. Enter a name for this new folder (I recommend "day03-in-class"), and choose a convenient location for it (I recommend creating a folder called 
...
Look at the file pane in the lower-right. You should see some files that were newly created by RStudio: `.gitgnore` and `day03-in-class.Rproj`.
